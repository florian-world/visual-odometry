...
function [keyF,Tresh]=magicKeyF(p_W_landmarks,position,lastKeyframePosition,tresh=0.1,degAdd=0.0873)
keyF=0;
avgD=0
lenD=length(p_W_landmarks);
for i=1:lenD
    D=pdist2(p_W_landmarks(:,i),t_C_W');
    avgD=avgD+D;
end
keyF=pdist2(position,lastKeyframePosition)/(avgD/lenD);
Tresh=false
