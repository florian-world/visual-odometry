function [R_C_W, t_C_W, best_inlier_mask, max_num_inliers_history, num_iteration_history] ...
    = ransacLocalizationP3P(matched_query_keypoints, corresponding_landmarks, K)
...
while num_iterations > i
    ...
    is_inlier = false(1,size(matched_query_keypoints,2));
    sol_nr = 0;
    
    % Count inliers of all solutions:
    for ii=1:4
        projected_points = projectPoints(...
            (R_C_W_guess(:,:,ii) * corresponding_landmarks) + ...
    ...
    if nnz(is_inlier) > max_num_inliers && ...
        ...
        % update best guess:
        R_C_W_best_guess = R_C_W_guess(:,:,sol_nr);
        t_C_W_best_guess = t_C_W_guess(:,:,sol_nr);
    end
...
if max_num_inliers == 0
    ...
    % Do not use DLT on inliers but rather the best guess from P3P
...
%     t_C_W = M_C_W(:, end);
     ...
     R_C_W = R_C_W_best_guess;
     t_C_W = t_C_W_best_guess;
end
